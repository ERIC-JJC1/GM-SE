...
#
# PGR 模型混合训练脚本
# [已修复 v2] 修复 InfeasibilityLoss ImportError 并调整损失调用
...
def main():
    ...
    # PhysicsInformedLoss 内部权重 (来自 train_tagru_weakly.py)
    ...
    # 获取 slack 位置 (用于 PhysicsInformedLoss)
    ...
    try:
        ...
        print(f"警告: grid.json 未在 {data_path} 找到。")
        net = None 
        # _, _, _, net, *_ = build_ieee33(create_test_case=False)

    if net is None:
        print("错误：无法加载电网拓扑。")
        return

    # ac_model = ACModel(net=net, device=device) # PhysicsInformedLoss 似乎不需要 ac_model
    
    # [!!! 已修复 !!!] 物理损失 (作用于 state_base)
    # 使用 train_tagru_weakly.py 中验证过的 PhysicsInformedLoss
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in pbar_train:
            ...
            # --- 计算混合损失 ---
            # [!!! 已修复 !!!] 准备 PhysicsInformedLoss 所需的 batch_gpu 字典
            batch_gpu = {
                "z": z_seq,
                "R": R_seq,
                "ztype": ztype_np # 传递 numpy 版本
            }
            # 调用物理损失 (注意 PhysicsInformedLoss 返回 loss, loss_dict)
